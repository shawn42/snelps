...
class SettingsDialog < Rubygoo::Dialog
  ...
  def initialize(opts)
    ...
    sound_check = Rubygoo::CheckBox.new :label=>"Sound enabled", :checked => @settings[:sound], :x=>30, :y=> 100, :w=>40, :h=>40, :relative=>true
    sound_check.on :checked do |c|
      @settings[:sound] = c.checked?
    end
    add sound_check

    fs_check = Rubygoo::CheckBox.new :label=>"Fullscreen", :checked => @settings[:fullscreen], :x=>30, :y=> 170, :w=>40, :h=>40, :relative=>true
    ...
    ok_button = Rubygoo::Button.new "OK", :x=>50,:y=>250,:relative=>true
